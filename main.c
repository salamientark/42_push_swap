...
/*   Updated: 2023/12/31 17:14:44 by dbaladro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int    main(int ac, char **av)
{
    ...
    // while (stack_size(ps_env.stack_a.head) == 3 && a_sorted(ps_env.stack_a.head) != 0)
    //     sort_stack_data_3(&(ps_env.stack_a.head));
    // print_stack(ps_env.stack_a, ps_env.stack_b, &get_elem_key, &print_stack_data);
    
    if (a_sorted(ps_env.stack_a.head) == 0)
        return (0);
    ft_printf("enter operation : ");
    op_str = get_next_line(0);
    index = 0;
    while (operation(&(ps_env.stack_a.head), &(ps_env.stack_b.head), op_str) && a_sorted(ps_env.stack_a.head) != 0)
    {
        index++;
